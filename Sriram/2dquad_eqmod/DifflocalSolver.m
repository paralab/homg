...
% $$$   %-------- End testing --------
  ...
  drhsu_dlam(:,index) = LIFT(:,:,f) * (diag(Jf .* taur)); %changed from previous implementation + instead of -
  ...
  uu  = uu  + taur * bdry;  %changed from previous implementation + instead of - 
  uqx = uqx -        bdryx;
  uqy = uqy -        bdryy;
end

uqx=-uqx;  %changed from previous implementation - instead of +
uqy=-uqy;  %changed from previous implementation - instead of +

qxMatrix = eMatInv * Kex;  %changed from previous implementation + instead of - 
% qxrhs = -eMatInv * rhsqx;
drhsqx_dlam = -eMatInv * drhsqx_dlam;  %changed from previous implementation - instead of +

qyMatrix = eMatInv * Key;  %changed from previous implementation + instead of -
%qyrhs = -eMatInv * rhsqy;
drhsqy_dlam = -eMatInv * drhsqy_dlam;  %changed from previous implementation - instead of +
