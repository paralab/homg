classdef jacobi
    ...
    methods(Static)
        function [P] = polynomial(x,alpha,beta,N)
            % function [P] = jacobi.polynomial(x,alpha,beta,N)
        ...
        function [dP] = gradient(r, alpha, beta, N)
            % function [dP] = jacobi.gradient(r, alpha, beta, N);
            ...
            if(N == 0)
                ...
                dP = sqrt(N*(N+alpha+beta+1)) * homg.jacobi.polynomial(r(:),alpha+1,beta+1, N-1); 
            end
        end
        
        function [x,w] = gauss(alpha,beta,N) 
            % function [x,w] = jacobi.gauss(alpha,beta,N)
        ...
        function [x] = gll(alpha,beta,N)
            % function [x] = jacobi.gll (alpha,beta,N)
            ...
            [xint,w] = homg.jacobi.gauss(alpha+1,beta+1,N-2);
            x = [-1, xint', 1]';
        end
