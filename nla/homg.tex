...
arising from higher order discretizations of elliptic partial
differential equations. In particular, our interest is on large-scale
problems with complex geometry and adaptively refined meshes, and the
implementation of these methods on parallel high performance computing
platforms. Thus, we favor (1) matrix-free methods, i.e., method that
do not require assembled finite element matrices, (2) methods that can
be parallelized on shared or distributed memory architectures, and (3)
...
geometries using highly adapted meshes.
