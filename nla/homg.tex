...
unknowns on the finest level by $N$, the cost for a matrix-vector
product is $Ng_p$, where $g_p>0$ is the cost per unknown for the
application of an operator originating from a finite element
approximation with polynomial order $p$. Since higher order
discretizations results in less sparse operators, $g_1\ge g_2\le
\ldots$. The actual value of $g_p$ depend on the architecture and the
implemenation.
