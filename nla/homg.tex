...
\begin{figure}
	\centering
		...
		\end{semilogyaxis}
		\draw[black, thick, fill=white, rounded corners] (4.0, 3.7) rectangle +(2.85,2);
		...
		\end{semilogyaxis}
		\draw[black, thick, fill=white, rounded corners] (3.8, 3.7) rectangle +(3.05,2);
		\draw[black] (4.0, 5.375) -- (6.85,5.375);
		\node at (5.35, 5.53) {\bf \small{vcycle, $p=1$}}; 
...
performance of our algorithms for the solution of high-order
discretizations of \eqref{eq:Poisson}.  After introducing our test
problems in Section~\ref{subsec:tests}, we describe in
Section~\ref{subsec:measures} how we compare the performance of
methods for high-order multigrid. The results for our test problem are
then presented and discussed Section~\ref{subsec:results}.


\subsection{Presentation of test problems}\label{subsec:tests}
We compare our algorithms to solve \eqref{eq:Poisson} in two and three
dimensions. We use a unit square and unit cube with constant
coefficients, as well as warped the two and three dimensional domains
with varying coefficient $\mu(\bs x)$, shown in
Figure~\ref{fig:mesh}. We use isoparametric elements, i.e., the
geometry transformation is approximated using the same polynomial
order as the finite element functions. The Jacobian is computed at
every quadrature point and \todo{say here how we deal with the varying
  coeffs on the coarser meshes?}
...
\item[$\bullet$] The columns entitled by \emph{MG as solver} report
  ...
  \end{itemize}
  Note that for each of the smoothers we report results for
  ...
  restrict ourselves to reporting orders that are powers of 2. After
  coarsening in $p$ as till $p=1$, and then coarsen in $h$. \todo{how
...
\item[$\bullet$] The columns headed by \emph{linearized pCG} present
  ...
  usually uses an algebraic multigrid cycle to approximately solve the
  linearized system, for simplicity we use a direct factorization
  method as solver for the low-order system. The different columns
  correspond to different to different numbers of Chebyshev smoothing
  steps on the finest mesh, where these smoothing steps use the
  high-order residual but the diagonal from the low-order system. Note
  that in this case Chebyshev requires an estimate of the largest
  eigenvalue of the high-order matrix preconditioned with the diagonal
  of the low-order operator.
\end{itemize}
