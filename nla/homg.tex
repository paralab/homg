...
Section~\ref{subsec:ptsmoothers}. We briefly comment on Schwarz-type
...
Chebyshev-accelerated Jacobi smoother~\cite{....}. These smoothers
require the diagonal of the system matrix, which has to be precomputed
in the setup if a the system matrices are not assembled. The SSOR
smoother is based on a lexicographic order or the unknowns.
\gsnote{Hari, do you think it is worth trying SSOR with a different
  node ordering to make it more red-black-like? We could simple
  reorder the matrix as we do for the low-order preconditioner.}  Note
that the parallelization of Gauss-Seidel smoothers (such as SSOR)
requires coloring of unknowns at parallel boundaries, and, compared to
Jacobi smoothing, more complex communication in a distributed memory
implementation.
