...
\title{Comparison of Geometric Multigrid Algorithms for High-order Continuous Finite Element Discretizations}
...
x)$. We use isoparametric elements for these deformed geometries,
i.e., the geometry transformation is approximated using the same
polynomial order as the finite element functions. The Jacobians for
this transformation are computed at every quadrature point. The
...
quadrature points. In our tests, change the polynomial degree of the
finite element functions but keep the same mesh; this results in an
increasing number of unknowns as $p$ increases.
...
methods converge for all polynomial orders in a relatively small
number of iterations. However, the number of iterations increases with
the polynomial orders $p$, in particular when multigrid is used as a
solver. Using multigrid as a preconditioner in CG results in a
reduction of multigrid v-cycles, in some cases even by a factor or
two. Also, we observe that SSOR smoothing outperforms the Jacobi-based
smoothers. We find the linear order operator based on the high-order
nodes to be a good preconditioner for the high-order system. The
convergence can be accelerated using smoothing steps on the finest
level. \gsnote{It's interesting that with 3 smoothing steps this
...
problem. \todo{Say something about the linearized preconditioner.}
