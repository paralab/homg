...
Given an order $p$, the operation count for a tensorized matrix-free matvec
is $\mathcal{O}(p^4)$ as opposed to $\mathcal{O}(p^6)$ for a fully assembled 
matrix (Orszag, JCP 37(1)  1980). Please note that this is only considering the dense elemental matrix,
so this is true for the best SpMV implementation possible. This has been
clarified and cited in the paper as well.

No, AMG was not tested. The text in the paper has been removed. 

Yes, in the current matlab implementation, the matrix is assembled. This
is the reason we are comparing the iteration counts (which is proportional to the
number of matvecs). 
